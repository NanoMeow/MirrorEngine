...
Locked files will be skipped when updating. Locking files is intended to be a
temporary measure, mirroring will pause for 5 minutes when a locked file is hit
(with default timer scale). This is designed this way to prevent too many
network requests being sent when many files are locked.
